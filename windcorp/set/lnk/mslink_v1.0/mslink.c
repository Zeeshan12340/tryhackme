...
/* LinkFlags */
char LinkFlags[] = "\x01\x01\x00\x00";
int LinkFlags_strlen = 4;
...
void getParameters(int argc, char *argv[]) {
	...
	while ((c = getopt (argc, argv, "hpl:o:")) != -1)
		switch (c)
		{
			case 'p':
				IS_PRINTER_LINK = 1;
...
void writeToFile() {
	...
	if (fichier != NULL) {
		...
		for (i = 0; i < LinkFlags_strlen ; i++) { fprintf(fichier, "%c", LinkFlags[i]); }
